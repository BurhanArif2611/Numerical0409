...
public class UserProfileHelper {
    ...
    private boolean isExist(UserProfileModel userDataModel) {
        read();
        Cursor cur = db.rawQuery("select * from " + UserProfileModel.TABLE_NAME + " where " + UserProfileModel.KEY_uid + "='" + userDataModel.getUid() + "'", null);
        if (cur.moveToFirst()) {
            return true;
        }
        return false;
    ...
    public void insertUserProfileModel(UserProfileModel userDataModel) {
        ...
        values.put(UserProfileModel.KEY_UserType, userDataModel.getUserType());
        values.put(UserProfileModel.KEY_VenderCode, userDataModel.getUserVenderCode());
        ...
        if (!isExist(userDataModel)) {
            ...
            db.update(UserProfileModel.TABLE_NAME, values, UserProfileModel.KEY_uid + "=" + userDataModel.getUid(), null);
        }
        close();
    }
    ...
    public ArrayList<UserProfileModel> getUserProfileModel() {
        ...
        if (clientCur != null && clientCur.getCount() > 0) {
            ...
            do {
                ...
                userDataModel.setUserType(clientCur.getString(clientCur.getColumnIndex(UserProfileModel.KEY_UserType)));
                userDataModel.setUserVenderCode(clientCur.getString(clientCur.getColumnIndex(UserProfileModel.KEY_VenderCode)));
