...
public class PaymentAmountDialog implements View.OnClickListener {

    private BottomInverseAlertLayoutBinding mBinding;
    private BottomSheetDialog mBottomSheetDialog;
    private Context mContext;
    private Calendar mCalendar = Calendar.getInstance();
    private static PaymentAmountDialog ourInstance;


    public static PaymentAmountDialog getInstance(final Context context, String message, String positiveText, String negativeText) {
        if (ourInstance == null) {
            synchronized (PaymentAmountDialog.class) {
                if (ourInstance == null) {
                    ourInstance = new PaymentAmountDialog(context, message, positiveText, negativeText);
                }
            }
        }
    ...
    private PaymentAmountDialog(final Context context, String message, String positiveText, String negativeText) {
        mContext = context;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.text_positive:
                EventBus.getDefault().post(new OnPaymentConfirmClicked());
                clearInstance();
                break;
